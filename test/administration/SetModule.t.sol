...
contract SetModulesTest is PrepareEachFunctionSuccessfulExecution {
    ...
    function modulesCalls(address user) public view returns (bytes[] memory) {
        bytes[] memory selectors = new bytes[](77);
        uint256 amount = 1000;
        ...
        selectors[76] = abi.encodeCall(ILTV.decimals, ());
        ...
        testWithPredefinedDefaultValues(data)
    {
        ...
        bytes[] memory moduleDependentCalls = new bytes[](18);
        moduleDependentCalls[0] = abi.encodeCall(ILTV.deposit, (1000, user));
        moduleDependentCalls[1] = abi.encodeCall(ILTV.mint, (1000, user));
        moduleDependentCalls[2] = abi.encodeCall(ILTV.redeem, (1000, user, user));
        moduleDependentCalls[3] = abi.encodeCall(ILTV.withdraw, (1000, user, user));
        moduleDependentCalls[4] = abi.encodeCall(ILTV.depositCollateral, (1000, user));
        moduleDependentCalls[5] = abi.encodeCall(ILTV.mintCollateral, (1000, user));
        moduleDependentCalls[6] = abi.encodeCall(ILTV.redeemCollateral, (1000, user, user));
        moduleDependentCalls[7] = abi.encodeCall(ILTV.withdrawCollateral, (1000, user, user));
        moduleDependentCalls[8] = abi.encodeCall(ILTV.approve, (user, 1000));
        moduleDependentCalls[9] = abi.encodeCall(ILTV.transfer, (user, 1000));
        moduleDependentCalls[10] = abi.encodeCall(ILTV.transferFrom, (address(0), user, 1000));
        moduleDependentCalls[11] = abi.encodeCall(ILTV.executeAuctionBorrow, (int256(1000)));
        moduleDependentCalls[12] = abi.encodeCall(ILTV.executeAuctionCollateral, (int256(1000)));
        moduleDependentCalls[13] = abi.encodeCall(ILTV.executeLowLevelRebalanceBorrow, (int256(1000)));
        moduleDependentCalls[14] = abi.encodeCall(ILTV.executeLowLevelRebalanceBorrowHint, (int256(1000), true));
        moduleDependentCalls[15] = abi.encodeCall(ILTV.executeLowLevelRebalanceCollateral, (int256(1000)));
        moduleDependentCalls[16] = abi.encodeCall(ILTV.executeLowLevelRebalanceCollateralHint, (int256(1000), true));
        moduleDependentCalls[17] = abi.encodeCall(ILTV.executeLowLevelRebalanceShares, (int256(1000)));
    ...
    /// forge-config: default.fuzz.runs = 10
    function test_eachModuleCallSucceedsWithValidModules(DefaultTestData memory data, address user)
        public
        testWithPredefinedDefaultValues(data)
    {
        vm.assume(user != data.feeCollector);
    ...
    /// forge-config: default.fuzz.runs = 10
    function test_eachModuleCallFailsWithDummyModules(DefaultTestData memory data, address user)
        public
        testWithPredefinedDefaultValues(data)
    {
        vm.assume(user != data.feeCollector);
        ...
        bytes[] memory failingCalls = new bytes[](6);
        failingCalls[0] = abi.encodeCall(ILTV.deposit, (1000, user));
        failingCalls[1] = abi.encodeCall(ILTV.mint, (1000, user));
        failingCalls[2] = abi.encodeCall(ILTV.depositCollateral, (1000, user));
        failingCalls[3] = abi.encodeCall(ILTV.mintCollateral, (1000, user));
        failingCalls[4] = abi.encodeCall(ILTV.approve, (user, 1000));
        failingCalls[5] = abi.encodeCall(ILTV.transfer, (user, 500));

        for (uint256 i = 0; i < failingCalls.length; i++) {
            vm.prank(user);
            vm.expectRevert(abi.encodeWithSelector(IAdministrationErrors.ZeroDataRevert.selector));
            (bool success,) = address(ltv).call{gas: gasleft()}(failingCalls[i]);
            success;
        }
    }

    function test_moduleStateValidation(DefaultTestData memory data) 
        public 
        testWithPredefinedDefaultValues(data) 
    {
        ModulesState memory validModulesState = ModulesState({
        ...
        vm.prank(testUser);
        (bool success,) = address(ltv).call(abi.encodeCall(ILTV.balanceOf, (testUser)));
        assertTrue(success);
        ...
        vm.prank(testUser);
        vm.expectRevert(abi.encodeWithSelector(IAdministrationErrors.ZeroDataRevert.selector));
        ltv.approve(testUser, 1000);
        
        vm.prank(testUser);
        (bool balanceSuccess,) = address(ltv).call(abi.encodeCall(ILTV.balanceOf, (testUser)));
        assertTrue(balanceSuccess);
    }

    function test_moduleVsNonModuleFunctionBehavior(DefaultTestData memory data, address user)
        public
        testWithPredefinedDefaultValues(data)
    {
        vm.assume(user != data.feeCollector);
        ...
        vm.prank(user);
        vm.expectRevert(abi.encodeWithSelector(IAdministrationErrors.ZeroDataRevert.selector));
        ltv.approve(user, 1000);
        
        vm.prank(user);
        vm.expectRevert(abi.encodeWithSelector(IAdministrationErrors.ZeroDataRevert.selector));
        ltv.transfer(user, 100);

        vm.prank(user);
        (bool success1,) = address(ltv).call(abi.encodeCall(ILTV.getLendingConnector, ()));
        assertTrue(success1);
        
        vm.prank(user);
        (bool success2,) = address(ltv).call(abi.encodeCall(ILTV.balanceOf, (user)));
        assertTrue(success2);
        
        vm.prank(user);
        (bool success3,) = address(ltv).call(abi.encodeCall(ILTV.decimals, ()));
        assertTrue(success3);
    }

    function test_moduleReplacement(DefaultTestData memory data) 
        public 
        testWithPredefinedDefaultValues(data) 
    {
        address testUser = address(0x456);
        
        ModulesState memory validModulesState1 = ModulesState({
            borrowVaultModule: IBorrowVaultModule(address(new BorrowVaultModule())),
            collateralVaultModule: ICollateralVaultModule(address(new CollateralVaultModule())),
            lowLevelRebalanceModule: ILowLevelRebalanceModule(address(new LowLevelRebalanceModule())),
            auctionModule: IAuctionModule(address(new AuctionModule())),
        ...
        ModulesProvider validModulesProvider1 = new ModulesProvider(validModulesState1);
        vm.prank(data.owner);
        ltv.setModules(IModules(address(validModulesProvider1)));
        
        vm.prank(testUser);
        (bool success1,) = address(ltv).call(abi.encodeCall(ILTV.balanceOf, (testUser)));
        assertTrue(success1);
        
        ModulesState memory validModulesState2 = ModulesState({
            borrowVaultModule: IBorrowVaultModule(address(new BorrowVaultModule())),
            collateralVaultModule: ICollateralVaultModule(address(new CollateralVaultModule())),
            lowLevelRebalanceModule: ILowLevelRebalanceModule(address(new LowLevelRebalanceModule())),
            auctionModule: IAuctionModule(address(new AuctionModule())),
        ...
        ModulesProvider validModulesProvider2 = new ModulesProvider(validModulesState2);
        vm.prank(data.owner);
        ltv.setModules(IModules(address(validModulesProvider2)));
        
        vm.prank(testUser);
        (bool success2,) = address(ltv).call(abi.encodeCall(ILTV.balanceOf, (testUser)));
        assertTrue(success2);
        ...
        vm.prank(testUser);
        (bool success4,) = address(ltv).call(abi.encodeCall(ILTV.balanceOf, (testUser)));
        assertTrue(success4);
    }
}