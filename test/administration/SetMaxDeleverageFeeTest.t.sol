...
contract SetMaxDeleverageFeeTest is BaseTest {
    function test_failIfNotEmergencyDeleverager(
    ...
    function test_failIfGreaterFeeInDeleverageAndWithdraw(DefaultTestData memory defaultData) public testWithPredefinedDefaultValues(defaultData) {
        ...
        vm.expectRevert(abi.encodeWithSelector(IAdministrationErrors.ExceedsMaxDeleverageFee.selector, deleverageFee, maxDeleverageFee));
        ltv.deleverageAndWithdraw(0, deleverageFee);
    }

    function test_worksWithLessFeeInDeleverageAndWithdraw(DefaultTestData memory defaultData) public testWithPredefinedDefaultValues(defaultData) {
        ...
        uint256 zeroFeeAssets = borrowAssets * ltv.oracleConnector().getPriceBorrowOracle() / ltv.oracleConnector().getPriceCollateralOracle();
    ...
    function test_canDeleverageWithZeroMaxFee(DefaultTestData memory defaultData) public testWithPredefinedDefaultValues(defaultData) {
        ...
        assertEq(
            ...
            (borrowAssets * ltv.oracleConnector().getPriceBorrowOracle()) / ltv.oracleConnector().getPriceCollateralOracle()
        );
    }
    ...
    function test_failIfOne(DefaultTestData memory defaultData) public testWithPredefinedDefaultValues(defaultData) {
        ...
        vm.startPrank(defaultData.governor);
        vm.expectRevert(abi.encodeWithSelector(IAdministrationErrors.InvalidMaxDeleverageFee.selector, newMaxDeleverageFee));
        ltv.setMaxDeleverageFee(newMaxDeleverageFee);
    }
