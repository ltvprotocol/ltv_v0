...
abstract contract PreviewWithdraw is Vault {
    ...
    function previewWithdraw(uint256 assets, VaultState memory state) public pure returns (uint256 shares) {
        return _previewWithdraw(assets, vaultStateToData(state));
    }

    function _previewWithdraw(uint256 assets, VaultData memory data) internal pure returns (uint256 shares) {
        ...
        return uint256(-sharesInUnderlying).mulDivUp(Constants.ORACLE_DIVIDER, data.borrowPrice).mulDivUp(data.supplyAfterFee, data.totalAssets);
    }
}
