...
contract PreviewRedeemCollateral is VaultCollateral {
    using uMulDiv for uint256;

    function previewRedeemCollateral(uint256 shares, PreviewVaultState memory state) public pure returns (uint256 assets) {
        (assets, ) = _previewRedeemCollateral(shares, previewVaultStateToPreviewCollateralVaultData(state, false));
    }

    function _previewRedeemCollateral(uint256 shares, PreviewCollateralVaultData memory data) internal pure returns (uint256, DeltaFuture memory) {
