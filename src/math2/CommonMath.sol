...
library CommonMath {
    ...
    function convertFutureCollateral(int256 futureCollateralAssets, uint256 collateralPrice, bool isDeposit) internal pure returns (int256) {
        // in case of deposit we need to assume more assets in the protocol, so round collateral up
        return futureCollateralAssets.mulDiv(int256(collateralPrice), int256(Constants.ORACLE_DIVIDER), isDeposit);
    }

    function convertFutureBorrow(int256 futureBorrowAssets, uint256 borrowPrice, bool isDeposit) internal pure returns (int256) {
        // in case of deposit we need to assume more assets in the protocol, so round borrow down
    ...
    function convertFutureRewardBorrow(int256 futureRewardBorrowAssets, uint256 borrowPrice, bool isDeposit) internal pure returns (int256) {
        // in case of deposit we need to assume more assets in the protocol, so round borrow down
    ...
    function calculateUserFutureRewardBorrow(int256 futureRewardBorrowAssets, uint256 auctionStep) internal pure returns (int256) {
        return futureRewardBorrowAssets.mulDivUp(int256(auctionStep), int256(Constants.AMOUNT_OF_STEPS));
    }

    // Fee collector <=> Auction executor conflict. Resolve it in favor of the auction executor.
    function calculateUserFutureRewardCollateral(int256 futureRewardCollateralAssets, uint256 auctionStep) internal pure returns (int256) {
